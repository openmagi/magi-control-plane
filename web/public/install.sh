...
# Issue #1 P1 (#17): Endpoint attestation. The installer seeds a stable
# endpoint id from the hostname so this host shows up on /endpoints
# the first time magi-cp-heartbeat runs.
MAGI_CP_ENDPOINT_ID=$(hostname -s 2>/dev/null || echo "host")-$(whoami)
MAGI_CP_ENDPOINT_LABEL=$(hostname -s 2>/dev/null || echo "host")
EOF
