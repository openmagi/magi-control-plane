 ...
 * attached script when a CC hook fires. Covers /scripts upload flow,
 * runtime selection, timeout, fail_closed semantics.
 */
...
export default function RunCommandPage() {
  ...
  "fail_closed": false
          ...
          <h3>2. 첨부 스크립트</h3>
          <p>
            긴 스크립트는 <Link href="/scripts">/scripts</Link> 에서 업로드합니다. 업로드하면 cloud
            가 sha256 + 메타데이터를 보관하고 정책에서는 script id 로 참조합니다.
          </p>
          <CodeBlock>{`{
  "kind": "run_command",
  "script_path": "fix-imports.py",
  "runtime": "python3",
  "timeout_seconds": 30,
  "fail_closed": true
}`}</CodeBlock>
          ...
          <ol>
            ...
            <li>파일을 선택, runtime 선택 (bash/python3/node), 설명 입력</li>
            <li>업로드 후 보이는 <b>script_path</b> 가 policy IR 에 넣을 값</li>
          </ol>

          <h2>runtime</h2>
          <p>
            허용 runtime 은 <Code inline>bash</Code>, <Code inline>sh</Code>,
            <Code inline>python3</Code>, <Code inline>node</Code> 네 가지입니다. 컨테이너 안에
            없는 runtime 을 선택하면 정책이 invalid 로 거부됩니다.
          </p>

          <h2>timeout</h2>
          <p>
            기본은 30초, 최대 120초. 초과하면 프로세스가 SIGKILL 됩니다.
            긴 작업은 fire-and-forget 로 백그라운드에 두세요 (<Code inline>nohup … &</Code>).
          </p>
  ...
  "fail_closed": false
          ...
          <p>
            ...
            stores the sha256 + metadata; the policy references it by script id.
          </p>
          <CodeBlock>{`{
  "kind": "run_command",
  "script_path": "fix-imports.py",
  ...
  "fail_closed": true
          ...
          <ol>
            ...
            <li>Pick the file, choose runtime (bash/python3/node), add a description.</li>
            <li>The <b>script_path</b> shown after upload is what goes into the policy IR.</li>
          </ol>

          <h2>Runtime</h2>
          <p>
            Allowed runtimes: <Code inline>bash</Code>, <Code inline>sh</Code>,
            <Code inline>python3</Code>, <Code inline>node</Code>. Choosing a runtime that
            isn't present in the container makes the policy invalid.
          </p>

          <h2>Timeout</h2>
          <p>
            Defaults to 30s, max 120s. The process is SIGKILLed on overflow. For long jobs,
            fire-and-forget into the background (<Code inline>nohup … &</Code>).
          </p>
