...
export const ENV_REFERENCE: ReadonlyArray<EnvVarEntry> = [
  ...
  {
    name: "MAGI_CP_GJC_RUNTIME_ENABLED",
    group: "cloud",
    default: "1",
    allowed: "0 | 1",
    ko: "gjc(gajae-code) CLI 런타임 어댑터의 전역 가용성 스위치(기본 켜짐). 켜져 있으면 런타임 선택기가 gjc 전환을 허용합니다. 단 이것은 가용성만 켤 뿐이고, 테넌트는 runtime_id가 gjc일 때만 실제로 gjc 경로를 탑니다. 명시적 falsy 값(0/false/off)으로 설정하면 어댑터가 꺼지고 다른 런타임 경로가 바이트 동일합니다(런타임별 독립 kill-switch).",
    en: "Global availability switch for the gjc (gajae-code) CLI runtime adapter (default ON). When on, the runtime picker allows a switch to gjc; this only makes gjc selectable (a tenant reaches the gjc path only when its runtime_id is gjc). Set an explicit falsy value (0/false/off) to disable the adapter; each runtime's kill switch is independent, so the other runtime paths stay byte-identical.",
  },
  {
  ...
  },
  {
    ...
    group: "local",
    default: "(auto)",
    allowed: "cc | codex | hermes | gjc",
    ko: "게이트 디스패처가 어떤 런타임 표준입력을 파싱할지 강제 지정. Codex/gjc managed config가 이 값을 각 런타임으로 고정합니다. 없으면 페이로드 스니핑 후 CC로 폴백.",
    en: "Forces which runtime stdin the gate dispatcher parses. Codex/gjc managed config pins this to their runtime; unset falls back to payload sniff then CC.",
  },
  {
    name: "MAGI_CP_GATE_BIN",
    group: "local",
    default: "magi-cp",
    ko: "gjc 훅 shim(session-start / session-shutdown / tool-call)이 호출하는 magi-cp gate 바이너리의 경로. 설치 시 절대 경로로 baked되며, 없으면 PATH의 `magi-cp`를 씁니다.",
    en: "Path to the magi-cp gate binary the gjc hook shims (session-start / session-shutdown / tool-call) invoke. Baked to an absolute path at install time; unset falls back to `magi-cp` on PATH.",
  },
  {
    name: "MAGI_CP_GJC_PLUGIN_DIR",
    group: "local",
    default: "~/.gjc/agent/gjc-plugins/magi-cp-gate",
    ko: "gjc 플러그인 번들 설치 디렉터리 오버라이드(테스트나 rootless 설치용). 없으면 기본 `~/.gjc/agent/gjc-plugins/magi-cp-gate`를 씁니다.",
    en: "Override for the gjc plugin-bundle install directory (tests / rootless installs). Unset uses the default `~/.gjc/agent/gjc-plugins/magi-cp-gate`.",
  },
  {
